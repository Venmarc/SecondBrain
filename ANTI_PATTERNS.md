...
| Decide "hardware vs software GL" by testing a `WEBGL_debug_renderer_info` string against a *software* regex (`/swiftshader\|llvmpipe\|…/`) | When the probe returns an empty or unrecognised string (failed context, privacy stripping, unusual driver) the regex is false → you conclude *hardware* and start heavy WebGL work inside the load window. Symptom seen: identical build scoring Lighthouse 99 then 68 across runs (TBT 5.8 s, no engine chunk even fetched) | Invert it: require a positively recognised hardware GPU (`nvidia\|geforce\|radeon\|amd\|intel\|apple\|adreno\|mali\|powervr\|…`), otherwise take the slow path. Cache the result for the page — creating the probe context itself can block for seconds under SwiftShader | [[06-Agent-Sessions/2026-09-14-opencode-antigravity-day-page-smoothscroll]] |

## GSAP (ScrollSmoother / ScrollTrigger)

| Never Do This | Why | Do This Instead | Source |
|---|---|---|---|
| Dynamically import `gsap` + `ScrollTrigger` + `ScrollSmoother` on component mount | ~127 kB lands before FCP and blocks first paint (measured: FCP 2.3 s, LCP 2.9 s, Performance 91); ScrollTrigger's initial refresh also forces ~46 ms of reflow | Gate initialisation on first interaction intent (`wheel`/`pointermove`/`touchstart`/`keydown`, `once`). Every path into scrolling raises one of these, so the smoother is live before the first real scroll — and a load that is never scrolled pays nothing | [[06-Agent-Sessions/2026-09-14-opencode-antigravity-day-page-smoothscroll]] |
| "Fix" the above by deferring the import on a `setTimeout` | Any timer lands the gsap parse/eval inside the TBT window; the long task also pushes TTI out, pulling more tasks into the measurement (measured: TBT 1.4 s, TTI 7.6 s, Performance 71) | Don't time-shift the cost — remove it from the non-interacting load entirely (interaction gate). If you must defer, defer past TTI *and* accept that the task still extends TTI | [[06-Agent-Sessions/2026-09-14-opencode-antigravity-day-page-smoothscroll]] |
| Put a page's themed `background` on the element that wraps a ScrollSmoother wrapper | ScrollSmoother sets the wrapper to `position: fixed` → the wrapper is out of flow → the parent collapses to zero height and paints nothing, so the white `body` shows through. Symptom: dark-theme light text became light-on-white and vanished | Give the page a separate full-viewport painted layer (e.g. `position: fixed; inset: 0`, `pointer-events: none`, carrying the theme colour), or set the background on `body`. Never rely on the smoother's own ancestors for viewport paint | [[06-Agent-Sessions/2026-09-14-opencode-antigravity-day-page-smoothscroll]] |
...
| Send raw IP addresses, email addresses, or full domain names in prompts to AgentRouter-backed models (Hermes `custom:agentrouter`, gpt-5.6-sol / claude-opus-4-8 / claude-opus-5) | AgentRouter's upstream content filter (Chinese-style 敏感词 moderation) rejects them — Hermes desktop surfaces it as `HTTP 500: sensitive words detected` after 3 retries; direct API returns `400 content-blocked`. Filter runs **before** auth (blocked tokens never even reach the key check). Trigger tokens confirmed by live bisection: `protonmail`, any email address, raw IP strings, `*.sslip.io`; safe: `ssh key`, `sudo password`, `telegram`, ports, names | Use aliases ("the oracle server", "my VPS", "my mail provider") or reference a local file instead of typing the literal token; or switch to a provider without the filter (DeepSeek, Nous Portal) for infra-sensitive chat | [[06-Agent-Sessions/2026-08-14-hermes-agentrouter-sensitive-words-block]] |
...
| Never Do This | Why | Do This Instead | Source |
|---|---|---|---|
| Let a tool timeout kill `nvm install <new> --reinstall-packages-from=<old>` mid-flight | npm finishes extracting packages into `lib/node_modules` but never creates the `bin/` symlinks — the new version "works" (`node -v` fine) while every global CLI (vercel, gemini, defuddle, agentmemory…) is silently missing. `nvm ls` and `npm ls -g` both look healthy | Re-run `nvm reinstall-packages <old>` detached (nohup/setsid, no tool timeout), then verify with `ls ~/.nvm/versions/node/<new>/bin/` — more than just node/npm/npx means links landed | [[06-Agent-Sessions/2026-09-12-opencode-node24-upgrade]] |
| Hardcode a versioned nvm path (`~/.nvm/versions/node/vX.Y.Z/bin`) in a systemd unit's `Environment="PATH=..."` | The pinned version shadows the nvm default forever: restarting the service keeps spawning the OLD node after an upgrade, and breaks entirely if that version is later uninstalled. `nvm alias default` changes have no effect on the unit | Update the unit PATH to the new version on every upgrade, then `systemctl --user daemon-reload && systemctl --user restart <unit>`; verify with `readlink /proc/<pid>/exe` on the spawned node process | [[06-Agent-Sessions/2026-09-12-opencode-node24-upgrade]] |
